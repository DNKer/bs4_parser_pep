...
# PATTERN - шаблон для поиска версии <version> и статуса <status>
# \d - цифра 0...9; \. - обычная точка; \d+ - одна или несколько
# цифр 0...9; \( и \) - скобки; .* - любой символ, кроме перевода строки;
# * - символ слева может отсутствовать или повторяться сколько угодно раз
PATTERN: str = r'Python (?P<version>\d\.\d+) \((?P<status>.*)\)'
# PDF_A4_TAG - шаблон для скачивания zip архива
